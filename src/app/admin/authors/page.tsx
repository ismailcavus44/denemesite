  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error } = await supabase
        ...
        .order("name");
      if (!error) setAuthors((data as Author[]) ?? []);
      setLoading(false);
    }
    load();
  }, []);
  ...
  return (
    <div className="rounded-2xl sm:rounded-[2rem] border border-slate-100 bg-white p-4 sm:p-6 lg:p-8 shadow-sm">
      <div className="mb-4 sm:mb-6 flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Yazarlar</h1>
          <p className="mt-0.5 text-sm text-slate-500">
            Toplam {authors.length} yazar
          </p>
        </div>
      ...
      {loading ? (
        ...
        <p className="py-12 text-sm text-slate-500">Henüz yazar yok.</p>
      ) : (
        <>
